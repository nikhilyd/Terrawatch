...
const PIPELINE_STAGES = [
  ...
  "Analyzing with GPT-4o Vision AI...",
  ...
  return (
    <div className="mt-2 flex flex-col gap-1.5">
      ...
      <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
        <motion.div
          className="h-full bg-gradient-to-r from-amber-500 via-cyan-500 to-blue-500 rounded-full"
...
export function ScanJobsPanel({ zone, scans, isLoading, onTriggerScan, onRetryScan, isTriggering }: ScanJobsPanelProps) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "completed": return <CheckCircle2 className="text-blue-500" size={16} />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "completed": return "text-blue-500 border-blue-500/20 bg-blue-500/10";
      ...
      <div className="p-5 border-b border-white/5 flex flex-col gap-3 relative overflow-hidden">
        ...
        {scans.some(s => s.status === "pending" || s.status === "processing") && (
          <div className="flex items-start gap-2 px-3 py-2 bg-amber-500/5 border border-amber-500/20 rounded-lg mt-1">
            ...
            <p className="text-[9px] font-mono text-amber-400/80 leading-relaxed">
              ML pipeline active — Sentinel-2 imagery fetch + GPT-4o Vision analysis takes <strong>5–10 minutes</strong>. Results will appear automatically below.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-blue-500/20 text-white hover:text-blue-400 border border-white/10 hover:border-blue-500/50 transition-all py-2 rounded-lg text-[10px] font-mono uppercase tracking-widest disabled:opacity-50"
                ...
                >
                  ...
                  {scan.status === 'completed' && scan.results && (() => {
                    ...
                    const severityColor: Record<string, string> = {
                      none:     "text-blue-400 bg-blue-500/10 border-blue-500/30",
                    ...
                    const landBars = [
                      { label: "Forest",  pct: r.forestPercentage,     color: "bg-blue-500" },
                      ...
                      <div className="mt-2 flex flex-col gap-3">
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                          <div className="bg-black/30 p-2 rounded-lg flex flex-col gap-0.5">
                            ...
                            <span className={`text-[10px] font-mono font-bold ${r.deforestationDetected ? "text-red-400" : "text-blue-400"}`}>
                          ...
                          <div className="bg-black/30 p-2 rounded-lg flex flex-col gap-0.5">
                            ...
                            <span className={`text-[10px] font-mono font-bold uppercase ${
                              r.vlConfidence === "high" ? "text-blue-400" :
