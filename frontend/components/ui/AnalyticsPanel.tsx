...
export function AnalyticsPanel({ analytics, alertsOverTime, riskScores }: AnalyticsPanelProps) {
  ...
  useEffect(() => {
    ...
    const checkML = async () => {
      try {
        const r = await fetch("http://localhost:5000/api/scan/health", { signal: AbortSignal.timeout(3000) });
        setMlOnline(r.ok);
  ...
  return (
    <div className="w-[320px] h-full flex flex-col gap-4 overflow-y-auto scrollbar-none pb-4">
      ...
      <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex flex-col gap-4">
        <h3 className="text-[10px] font-mono tracking-widest uppercase text-blue-500">System Telemetry</h3>
        
        <div className="grid grid-cols-2 gap-3">
          {/* Real API Latency */}
          <div className="bg-white/5 border border-white/5 rounded-lg p-3 flex flex-col gap-2">
            <SignalHigh size={16} className={apiLatency !== null && apiLatency < 100 ? "text-blue-400" : "text-amber-400"} />
          ...
          {/* Real Zone Count */}
          <div className="bg-white/5 border border-white/5 rounded-lg p-3 flex flex-col gap-2">
            <TreePine size={16} className="text-blue-400" />
      ...
      <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h3 className="text-[10px] font-mono tracking-widest uppercase text-blue-500">Monitoring Campaigns</h3>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="flex flex-col items-center p-2 bg-blue-500/5 border border-blue-500/15 rounded-lg">
            <span className="text-lg font-bold text-blue-400">{campaigns.active}</span>
      ...
      {avgForestPct !== null && (
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
          <h3 className="text-[10px] font-mono tracking-widest uppercase text-blue-500">Global Forest Health</h3>
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] font-mono text-zinc-400">Avg Coverage</span>
            <span className={`text-sm font-bold font-mono ${
              avgForestPct > 60 ? 'text-blue-400' : avgForestPct > 35 ? 'text-amber-400' : 'text-red-400'
            }`}>{avgForestPct}%</span>
          </div>
          <div className="w-full bg-white/5 rounded-full h-2 overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-700 ${
                avgForestPct > 60 ? 'bg-blue-500' : avgForestPct > 35 ? 'bg-amber-500' : 'bg-red-500'
              }`}
      ...
      {threatTypes.length > 0 && (
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[10px] font-mono tracking-widest uppercase text-blue-500">Threat Type Breakdown</h3>
      ...
      <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex-1 flex flex-col">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[10px] font-mono tracking-widest uppercase text-blue-500">Global Threat Matrix</h3>
          <ShieldAlert size={14} className="text-zinc-500" />
        </div>
        ...
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {recentAnalyses.map((a: any, i: number) => {
              ...
              const sevColor = sev === 'critical' ? 'text-red-400' : sev === 'high' ? 'text-amber-400' : sev === 'medium' ? 'text-yellow-400' : 'text-blue-400';
              return (
      ...
      <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex flex-col min-h-[250px]">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[10px] font-mono tracking-widest uppercase text-blue-500">6-Month Trend</h3>
