  ...
  return (
    <div className="flex items-end gap-px h-8">
      {Array.from({ length: 20 }, (_, i) => (
        <div key={i}
          className={`w-1.5 rounded-t-sm ${i < bars ? "bg-blue-500" : "bg-white/5"}`}
          style={{ height: `${20 + i * 3}%` }}
        />
      ))}
    </div>
    ...
    <div className="relative h-32 flex items-end gap-2 px-2">
      ...
      <div className="flex-1 ml-8 flex items-end gap-1.5 h-full">
        {done.map((scan, i) => {
          ...
          const color   = isFirst ? "bg-blue-500" : drop >= 10 ? "bg-red-500" : drop >= 5 ? "bg-yellow-500" : "bg-blue-500";
          return (
              ...
              <div className="relative flex-1 w-full flex items-end">
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block z-10">
                  <div className="bg-gray-900 border border-white/10 rounded-lg px-2 py-1.5 text-[10px] text-white whitespace-nowrap shadow-xl">
                    ...
                    {!isFirst && <div className={drop > 0 ? "text-red-300" : "text-blue-300"}>{drop > 0 ? `-${drop.toFixed(1)}%` : `+${Math.abs(drop).toFixed(1)}%`}</div>}
                  </div>
                </div>
              </div>
...
function SavedAnalysisCard({
  ...
  return (
    ...
    >
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${isLoss ? 'bg-red-500' : 'bg-blue-500'}`} />
      ...
      <div className="p-5 pl-6 flex flex-col md:flex-row md:items-center justify-between cursor-pointer gap-4" onClick={() => onExpand(analysis._id)}>
        ...
        <div className="flex gap-8 items-center md:pr-8">
          <div className="text-right">
            ...
            <p className={`text-base font-mono font-bold ${isLoss ? 'text-red-400' : 'text-blue-400'}`}>
      ...
      {analysis.ai_verdict && !expanded && (
        <div className="px-6 pb-4">
          <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-950/20 border border-blue-500/10">
            ...
            <p className="text-xs text-blue-100/70 italic line-clamp-2 leading-relaxed">
              {analysis.ai_verdict}
            </p>
          </div>
        </div>
        ...
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <div className="p-6 border-t border-white/5 space-y-6">
              ...
              {analysis.ai_verdict && (
                <div className="flex items-start gap-3 p-4 rounded-xl bg-blue-950/30 border border-blue-500/20 mb-6">
                    ...
                    <div className="p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                      ...
                      {i > 0 && scan.delta_from_first != null && (
                        <div className="mt-4 flex items-center gap-2">
                          <TrendingDown className={`w-4 h-4 ${scan.delta_from_first > 0 ? "text-red-400" : "text-blue-400"}`} />
                          <p className={`text-xs font-bold ${scan.delta_from_first > 0 ? "text-red-400" : "text-blue-400"}`}>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              {result.ai_verdict && (
                <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/20">
                  <p className="text-xs text-blue-400 font-semibold uppercase tracking-wider mb-2">AI Verdict</p>
