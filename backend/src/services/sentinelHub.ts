...
interface SentinelHubImage {
  rgb:      Buffer;    // RGB composite (B04+B03+B02) as PNG
  nir:      Float32Array;  // B08 raw values
  redRaw:   Float32Array;  // B04 raw values
  scl:      Uint8Array;    // Scene Classification Layer
  width:    number;
  height:   number;
  metadata: {
    bbox:      number[];
...
async function getAccessToken(): Promise<string> {
  ...
  const res = await axios.post(SH_TOKEN_URL, params.toString(), {
...
export async function fetchImage(params: {
  ...
  resolution?: number;    // meters/pixel (default 20)
...
function evaluatePixel(sample) {
  ...
  const res = await axios.post(SH_PROCESS_URL, requestBody, {
    headers: {
...
function parseMultipartResponse(body: Buffer, contentType: string): MultipartPart[] {
...
function parseTiffFloat32(buf: Buffer): { data: Float32Array; width: number; height: number } {
...
function parseTiffUint8(buf: Buffer): { data: Uint8Array; width: number; height: number } {
  const { width, height, stripOffset, stripByteCount } = readTiffHeaders(buf);
  const data = buf.subarray(stripOffset, stripOffset + stripByteCount);
  return { data: new Uint8Array(data), width, height };
}

function readTiffHeaders(buf: Buffer): {
  width: number; height: number; stripOffset: number; stripByteCount: number;
} {
  const isLittle = buf[0] === 0x49; // 'I' = little-endian
  const readU16 = (off: number) => isLittle
    ? buf.readUInt16LE(off)
    : buf.readUInt16BE(off);
  const readU32 = (off: number) => isLittle
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    switch (tag) {
      ...
      case 273: stripOffset = value; break;
      case 279: stripByteCount = value; break;
    }
  }
