...
import { analyzeImage, generateVerdict } from '../services/visionAnalysis';
import { saveImage, imageToBase64 } from '../services/imageService';
import { calculateLossHectares, calculateAnnualRate } from '../services/geoCalculator';

export const analyzeHistorical = async (req: Request, res: Response): Promise<void> => {
  try {
    const { zone_id, bbox, dates, resolution = 20 } = req.body;
    ...
    const results: any[] = [];
    let firstForestPct: number | null = null;
    ...
    for (const dateStr of dates) {
      ...
      try {
        ...
        let imageData = await fetchImage({ bbox, dateFrom, dateTo: dateStr, resolution });
        ...
        if (imageData.scl) {
          ...
          for (let i = 0; i < imageData.scl.length; i++) {
            if (CLOUD_SCL.has(imageData.scl[i])) cloudCount++;
          }
        ...
        if (cloudPct > MAX_CLOUD_PCT) {
          ...
          try {
            imageData = await fetchImage({ bbox, dateFrom: retryFrom, dateTo: retryTo, resolution });
            if (imageData.scl) {
              let cloudCount = 0;
              for (let i = 0; i < imageData.scl.length; i++) {
                if (CLOUD_SCL.has(imageData.scl[i])) cloudCount++;
        ...
        const jobId = `hist-${zone_id.slice(0, 8)}-${dateStr}-${uuidv4().slice(0, 4)}`;
        ...
        if (firstForestPct === null) firstForestPct = forestPct;
        const delta = Math.round(((firstForestPct || forestPct) - forestPct) * 100) / 100;
        const lossHa = calculateLossHectares(bbox, firstForestPct || forestPct, forestPct);
    ...
    if (doneScans.length >= 2) {
      ...
      totalLossHa = calculateLossHectares(bbox, doneScans[0].forest_pct, doneScans[doneScans.length - 1].forest_pct);
      ...
      ratePerYear = calculateAnnualRate(totalLossHa, days);

      for (let i = 1; i < doneScans.length; i++) {
        const drop = doneScans[i - 1].forest_pct - doneScans[i].forest_pct;
        if (drop > biggestDrop) { biggestDrop = drop; biggestDate = doneScans[i].date; }
      }
    }
    ...
    if (doneScans.length >= 2) {
      const allThreats = [...new Set(doneScans.flatMap((s: any) => s.threats))];
      const verdictPrompt =
        `Historical satellite analysis of ${doneScans.length} scans over ` +
      ...
      try {
        aiVerdict = await generateVerdict(verdictPrompt);
      } catch {
        if (totalLossPct > 20) aiVerdict = `Critical deforestation: ${totalLossPct}% loss. Immediate investigation needed.`;
        else if (totalLossPct > 5) aiVerdict = `Moderate forest loss: ${totalLossPct}%. Monitoring should continue.`;
        else aiVerdict = `Minimal change (${totalLossPct}%). Area appears stable.`;
      }
    }
