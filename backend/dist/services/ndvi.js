...
exports.generateNdviHeatmap = generateNdviHeatmap;
/**
 ...
 */
function calculateNdvi(nir, redRaw, scl) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        const denom = n + r;
        if (denom === 0)
            continue; // skip zero-division pixels
 ...
 */
async function generateNdviHeatmap(rgbBuf, nir, width, height) {
    ...
    for (let i = 0; i < pixelCount; i++) {
        ...
        if (t < 0.5) {
            ...
            const s = t * 2;
            r = 220;
            g = Math.round(60 + 140 * s);
            b = 60;
        }
        else {
            // Yellow → Green
            const s = (t - 0.5) * 2;
            r = Math.round(220 - 160 * s);
