...
exports.downloadFIRReport = exports.getAllRiskScores = exports.getZoneRiskScore = exports.getCarbonLoss = void 0;
...
const calcCarbonLoss = (deforestedKm2) => {
    ...
    const valueLakhINR = parseFloat((valueINR / 100000).toFixed(2));
    return { treesLost: Math.round(treesLost), co2Tonnes, valueUSD, valueINR, valueLakhINR };
};
// ── Helper: Risk Score ───────────────────────────────────────────────────────
const calcRiskScore = (ndviLossPct, alertCount, latestSeverity) => {
    const ndviScore = Math.min(40, ndviLossPct * 2.5);
    const alertScore = Math.min(30, alertCount * 5);
    const sevMap = { none: 0, low: 8, medium: 15, high: 23, critical: 30 };
...
const getHistoricalData = async (zoneId) => {
    const analysis = await HistoricalAnalysis_1.default
        .findOne({ zoneId })
        .sort({ createdAt: -1 });
    if (!analysis)
        return null;
    const doneScan = analysis.scans.filter((s) => s.status === 'done');
    if (doneScan.length < 1)
        return null;
    return {
        analysis,
        doneScan,
...
const getCarbonLoss = async (req, res) => {
    try {
...
exports.getCarbonLoss = getCarbonLoss;
// ── Feature 3: Zone Risk Score ───────────────────────────────────────────────
// GET /api/legal/zone/:id/risk
const getZoneRiskScore = async (req, res) => {
    try {
        const zone = await Zone_1.default.findById(req.params.id);
        if (!zone) {
            res.status(404).json({ success: false, message: 'Zone not found' });
        ...
        const alertCount = await Alert_1.default.countDocuments({ zoneId: req.params.id, createdAt: { $gte: since3months } });
        // ── Primary: HistoricalAnalysis ──────────────────────────────────────────
        const hist = await getHistoricalData(req.params.id);
        ...
        const latestThreats = (hist?.latestScan?.threats ?? []).filter((t) => t && t !== 'none');
        res.json({
            success: true,
            data: {
                zone: { id: zone._id, name: zone.name, area_km2: zone.area_km2 ?? 0 },
                ...
                recommendation: risk.level === 'CRITICAL' ? 'IMMEDIATE ground inspection and legal action required!' :
...
exports.getZoneRiskScore = getZoneRiskScore;
// ── GET /api/legal/risk-scores — All zones ranked by risk ───────────────────
const getAllRiskScores = async (req, res) => {
    try {
        const zones = await Zone_1.default.find({ isActive: true, createdBy: req.user?.id });
        const since3months = new Date();
        since3months.setMonth(since3months.getMonth() - 3);
        const results = await Promise.all(zones.map(async (zone) => {
            const [hist, alertCount] = await Promise.all([
                HistoricalAnalysis_1.default
                    .findOne({ zoneId: zone._id })
                    .sort({ createdAt: -1 })
                    .select('summary scans ai_verdict'),
                Alert_1.default.countDocuments({ zoneId: zone._id, createdAt: { $gte: since3months } }),
            ]);
            const doneScan = (hist?.scans ?? []).filter((s) => s.status === 'done');
...
exports.getAllRiskScores = getAllRiskScores;
// ── Feature 2: FIR-Ready Legal Evidence Report ──────────────────────────────
// GET /api/legal/zone/:id/fir
const downloadFIRReport = async (req, res) => {
    try {
        ...
        const [hist, alerts, fieldReports] = await Promise.all([
            ...
            Alert_1.default.find({ zoneId: req.params.id }).sort({ createdAt: -1 }).limit(10),
            FieldReport_1.default.find({ zoneId: req.params.id, status: 'analyzed' })
                .sort({ createdAt: -1 }).limit(5),
        ]);
        // Fallback scan data
        const scans = hist ? [] : await Scan_1.default.find({ zoneId: req.params.id, status: 'completed' })
        ...
        // ── PDF Setup ─────────────────────────────────────────────────────────────
        const doc = new pdfkit_1.default({ margin: 55, size: 'A4' });
        ...
        doc.fontSize(9).font('Helvetica-Bold').fillColor(RED)
            ...
            .text(`AI Model: GPT-4o Vision + NDVI + SCL Cloud Masking`, 300, ch + 37);
        doc.y = ch + 60;
        // ── Helpers ──────────────────────────────────────────────────────────────
        const section = (title) => {
            doc.moveDown(0.5);
        ...
        if (hist && hist.doneScan.length >= 2) {
            ...
            hist.doneScan.forEach((s, i) => {
                const threats = (s.threats ?? []).filter((t) => t !== 'none').join(', ') || 'None';
            ...
            scans.slice(0, 8).forEach((s, i) => {
                const threats = (s.results?.threats ?? []).filter(t => t !== 'none').join(', ') || 'None';
                doc.fontSize(7.5).font('Helvetica').fillColor(DARK)
        ...
        const threatsArr = (latestForSection?.threats ?? []).filter((t) => t !== 'none');
        ...
        if (alerts.length > 0) {
            ...
            alerts.forEach((a, i) => {
                const srcBadge = a.source === 'field_report' ? ' [GROUND INTEL]' : ' [SATELLITE]';
        ...
        if (fieldReports.length > 0) {
            ...
            fieldReports.forEach((fr, i) => {
                ...
                const threats = (fr.aiAnalysis?.threats ?? []).filter((t) => t !== 'none').join(', ') || 'None';
                const isCritical = sev === 'HIGH' || sev === 'CRITICAL';
                doc.fontSize(8).font('Helvetica-Bold').fillColor(isCritical ? RED : DARK)
                    .text(`  Report ${i + 1}: [${sev}] — ` +
                    `${new Date(fr.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}`, 65);
                doc.fontSize(7.5).font('Helvetica').fillColor(DARK)
                ...
                if (fr.aiAnalysis?.description) {
                    doc.fontSize(7.5).font('Helvetica').fillColor(GRAY)
                        .text(`  GPT-4o Vision Analysis: ${fr.aiAnalysis.description}`, 75, doc.y, { width: doc.page.width - 150 });
                }
                doc.moveDown(0.4);
        ...
        doc.moveDown(0.3);
        doc.text('AI Vision: GPT-4o Vision model performs natural language analysis of satellite imagery to identify threats (illegal logging, encroachment, mining). Analysis corroborates NDVI calculations.', 65, doc.y, { width: doc.page.width - 130, lineGap: 2 });
        ...
        doc.fontSize(8.5).font('Helvetica').fillColor(DARK)
            .text('This report is generated by EcoWatch — an AI-powered satellite forest monitoring system using European Space Agency (ESA) Sentinel-2 L2A imagery with SCL cloud masking and GPT-4o Vision analysis.', 65, doc.y, { width: doc.page.width - 130, lineGap: 3 });
        doc.moveDown(0.5);
        ...
        doc.fillColor('#aaa').fontSize(7)
            .text(`Case Ref: ${caseRef}  |  Generated: ${today}  |  EcoWatch AI Deforestation Monitor  |  Sentinel-2 L2A + SCL + GPT-4o Vision`, 55, doc.page.height - 20, { align: 'center', width: doc.page.width - 110 });
        doc.end();
    }
    catch (err) {
        if (!res.headersSent)
...
exports.downloadFIRReport = downloadFIRReport;
