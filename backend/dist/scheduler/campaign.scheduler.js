...
exports.startCampaignScheduler = exports.processCampaignScanResult = void 0;
const node_cron_1 = __importDefault(require("node-cron"));
const nodemailer_1 = __importDefault(require("nodemailer"));
const uuid_1 = require("uuid");
const Campaign_1 = __importDefault(require("../models/Campaign"));
const Scan_1 = __importDefault(require("../models/Scan"));
const scanProcessor_1 = require("./scanProcessor");
const env_1 = __importDefault(require("../config/env"));
const formatDate = (d) => d.toISOString().split('T')[0];
// ── Hectares calculator ───────────────────────────────────────────────────────
const calculateHectaresLost = (bbox, forestPctOld, forestPctNew) => {
...
// ── Email alert sender ────────────────────────────────────────────────────────
const sendDeforestationAlert = async (toEmail, campaignName, zoneName, scanIdx, deltaPct, lossHa) => {
...
const processCampaignScanResult = async (campaignId, scanIdx, scanDocId) => {
    try {
        const campaign = await Campaign_1.default.findById(campaignId).populate('zoneId', 'name');
        if (!campaign)
            return;
        const scan = await Scan_1.default.findById(scanDocId);
        if (!scan || scan.status !== 'completed')
            return;
        const zoneName = campaign.zoneId?.name ?? 'Unknown';
        ...
        campaign.scans[scanIdx].scanId = scan._id;
        ...
        if (!isBaseline &&
            ...
            await sendDeforestationAlert(campaign.alertEmail, campaign.name, zoneName, scanIdx, deltaFromPrevious, lossHectares);
        }
...
exports.processCampaignScanResult = processCampaignScanResult;
// ── Main: Trigger due campaign scans ─────────────────────────────────────────
const runCampaignScans = async () => {
    try {
        const now = new Date();
        const campaigns = await Campaign_1.default.find({ status: 'active' }).populate('zoneId');
        for (const campaign of campaigns) {
            const nextIdx = campaign.currentScanIdx;
            if (nextIdx >= campaign.scans.length)
                continue;
            const nextScan = campaign.scans[nextIdx];
            if (!nextScan || nextScan.status !== 'pending')
                continue;
            if (nextScan.scheduledDate > now)
                continue; // Not due yet
            // Mark as processing
            campaign.scans[nextIdx].status = 'processing';
            await campaign.save();
            const zone = campaign.zoneId;
            if (!zone || !campaign.bbox?.length)
                continue;
            ...
            (0, scanProcessor_1.processScanJob)(job).catch(err => console.error(`[${jobId}] Campaign scan failed:`, err));
...
const startCampaignScheduler = () => {
    ...
    setTimeout(runCampaignScans, 15000);
